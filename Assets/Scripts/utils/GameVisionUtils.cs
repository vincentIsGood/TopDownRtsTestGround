...
public class GameVisionUtils{
    ...
    public static bool canSeeTarget(Squad from, Soldier to){
        int layerMask = from.config.enemyMask | from.config.wallMask;
        Vector3 direction = (to.transform.position - from.center).normalized;
    ...
    public static bool canSeeTarget(Squad from, Soldier to, out RaycastHit2D hit){
        int layerMask = from.config.enemyMask | from.config.wallMask;
        Vector3 direction = (to.transform.position - from.center).normalized;
